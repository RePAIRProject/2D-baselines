...
from src.assembler.mock_springs_server import mock_simulator
...
# Use mock simulator by default, but allow switching to real server
USE_MOCK_SERVER = os.environ.get('USE_MOCK_SPRINGS_SERVER', 'true').lower() == 'true'
...
def simulate(matings, fixed_rotation={}, screenshot_name="", isInteractive=False, isDebug=False, collision="Off"):
    ...
    if USE_MOCK_SERVER:
        # Use the mock simulator instead of the HTTP server
        print("Using mock springs server (bypass mode)")
        response = mock_simulator.simulate(matings_as_list, fixed_rotation)
        return response
    else:
        # Use the real HTTP server
        print("Using real springs server")
        body = {
